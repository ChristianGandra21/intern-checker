...
export async function GET(_: NextRequest, context: { params: Promise<{ id: string }> }) {
  ...
  const result = await getSupabaseAdmin().from("tracked_applications").select("*,application_stages(*)").eq("id", id).eq("user_id", user.id).is("deleted_at", null).maybeSingle();
...
export async function PATCH(request: NextRequest, context: { params: Promise<{ id: string }> }) {
  ...
  for (const [key, max] of [["title", 240], ["company", 200], ["location", 240], ["description", 10000], ["notes", 10000]] as const) {
