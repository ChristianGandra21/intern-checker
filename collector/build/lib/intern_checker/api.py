...
async def send_to_api(
    ...
    run_payload = {
        ...
        "scrape_run_id": os.getenv("LOCAL_SCRAPE_RUN_ID") or os.getenv("SCRAPE_RUN_ID"),
        "official_snapshots": [
    ...
    result = IngestionResult(totals={key: 0 for key in (
        "accepted", "review", "rejected", "hidden", "resolved", "failures", "created", "updated", "duplicates"
    )})
    source_totals: dict[str, dict[str, int | str]] = defaultdict(
        lambda: {key: 0 for key in (
            "discovered", "resolved", "persisted", "accepted", "review", "rejected", "hidden", "failures",
    ...
    async with aiohttp.ClientSession() as session:
        async with session.post(
            runs_url,
            json={
                "found_count": run_payload["found_count"],
                "source_summary": source_summary,
                "scrape_run_id": run_payload["scrape_run_id"],
            },
            headers={"x-ingest-key": key},
            ...
            for batch in batches:
                ...
                for outcome in data.get("outcomes", []):
                    ...
                    if outcome.get("created"):
                        source_totals[source]["created"] += 1
                    if outcome.get("updated"):
                        source_totals[source]["updated"] += 1
                    if outcome.get("duplicate"):
                        source_totals[source]["duplicates"] += 1
