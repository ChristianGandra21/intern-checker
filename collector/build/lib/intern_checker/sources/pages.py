...
def _context_text(link) -> str:
    candidates = [link.get_text(" ", strip=True)]
    parent = link
    for _ in range(4):
        parent = parent.find_parent(["article", "li", "section", "div"])
        if not parent:
            break
        text = " ".join(parent.get_text(" ", strip=True).split())
        if text:
            candidates.append(text)
    relevant = [text for text in candidates if _looks_relevant(text, DEFAULT_INCLUDE_TERMS)]
    return min(relevant or candidates, key=lambda text: (len(text) < 20, len(text)))


def _candidate_title(link_text: str, context: str, page: dict) -> str:
    pattern = page.get("title_pattern")
    if pattern:
        match = re.search(pattern, context, re.IGNORECASE)
        if match:
            return (match.group(1) if match.lastindex else match.group()).strip()
    return link_text
...
async def _collect_page(session: aiohttp.ClientSession, page: dict) -> list[JobCandidate]:
    ...
    for link in soup.select(page.get("link_selector", "a[href]")):
        ...
        title = _candidate_title(link_text, context, page)
        ...
        jobs.append(
            JobCandidate(
                ...
                raw_payload={"discovery_page": url, "discovery_context": context[:2000]},
            )
        )
        if len(jobs) >= limit:
            break
