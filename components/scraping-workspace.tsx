...
import { CheckCircle2, Clock3, LoaderCircle, Play, SearchCheck, TriangleAlert } from "lucide-react";
...
function date(value: string | null) { return value ? new Intl.DateTimeFormat("pt-BR", { dateStyle: "short", timeStyle: "medium" }).format(new Date(value)) : "—"; }
...
export function ScrapingWorkspace({ initial, enabled }: { initial: ScrapeRun[]; enabled: boolean }) {
  ...
  return <div className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_340px]">
    <section className="surface overflow-hidden"><div className="flex flex-wrap items-center justify-between gap-4 border-b border-[var(--line)] bg-white p-5"><div><h2 className="text-2xl font-semibold tracking-[-.04em]">Histórico local</h2><p className="mt-1 text-sm text-[var(--ink-soft)]">Atualização automática durante a execução.</p></div>{active && <span className="eyebrow inline-flex items-center gap-2 text-[var(--blue)]"><LoaderCircle className="animate-spin" size={16} />Pipeline em andamento</span>}</div>
      {!runs.length ? <div className="grid min-h-64 place-items-center p-8 text-center"><div><SearchCheck className="mx-auto text-[var(--green)]" size={34} /><p className="mt-4 font-semibold">Nenhuma coleta manual registrada</p></div></div> : <div className="divide-y divide-[var(--line)]">{runs.map((run) => { const meta = statusMeta[run.status]; const Icon = meta.icon; return <article key={run.id} className="grid gap-4 p-5 md:grid-cols-[150px_1fr_130px] md:items-start"><div className={`flex items-center gap-2 font-semibold ${meta.className}`}><Icon className={run.status === "running" ? "animate-spin" : ""} size={18} />{meta.label}</div><div><p className="mono text-xs text-[var(--ink-soft)]">{run.id}</p><p className="mt-2 text-sm">Solicitada em {date(run.requested_at)}</p>{run.error_message && <p className="mt-2 text-sm font-medium text-[var(--red)]">{run.error_message}</p>}{run.summary && <details className="mt-3"><summary className="cursor-pointer text-sm font-semibold text-[var(--green)]">Ver resumo técnico</summary><pre className="mt-2 max-h-56 overflow-auto whitespace-pre-wrap bg-[var(--ink)] p-3 text-xs leading-relaxed text-white/75">{run.summary}</pre></details>}</div><div className="text-sm text-[var(--ink-soft)]"><p>Duração</p><p className="mono mt-1 text-[var(--ink)]">{duration(run)}</p></div></article>; })}</div>}
    </section>
    <aside className="surface h-fit overflow-hidden xl:sticky xl:top-6"><div className="bg-[var(--ink)] p-6 text-white"><p className="eyebrow text-[var(--acid)]">Ação administrativa</p><h2 className="mt-2 text-2xl font-semibold">Atualizar radar</h2><p className="mt-3 text-sm leading-relaxed text-white/65">Executa coleta, ingestão e revalidação usando o ambiente Python desta máquina.</p></div><div className="p-6"><button type="button" onClick={start} disabled={!enabled || active || busy} className="button-dark w-full">{busy || active ? <LoaderCircle className="animate-spin" size={18} /> : <Play size={18} />} {active ? "Coleta em andamento" : "Rodar scraping agora"}</button><p className="mt-3 text-xs leading-relaxed text-[var(--ink-soft)]">{enabled ? "Há um intervalo mínimo de 30 minutos e apenas uma execução simultânea." : "A coleta local está desabilitada nas variáveis de ambiente."}</p>{message && <p className={`mt-4 border-l-4 p-3 text-sm ${/iniciada/.test(message) ? "border-[var(--green)] bg-[#e8f5ef]" : "border-[var(--red)] bg-[#fff0ee]"}`} role="status">{message}</p>}</div></aside>
  </div>;
}
