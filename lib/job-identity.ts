...
export interface DedupIdentity { key: string; confidence: number; reasons: string[]; cycle: string | null; tokens: string[] }
...
export function buildDedupIdentity(job: IdentityInput): DedupIdentity {
  ...
  if (job.external_id && bestUrl) {
    ...
    return { key: digest(`external:${host}:${job.external_id}`), confidence: 100, reasons: ["mesmo identificador externo"], cycle, tokens };
  }
  if (bestUrl) return { key: digest(`official:${normalizedUrl(bestUrl)}`), confidence: 98, reasons: ["mesma URL oficial"], cycle, tokens };
  if (isProgram(job.title) && tokens.length) {
    return { key: digest(`program:${cycle || "unknown"}:${tokens.join("|")}`), confidence: company ? 92 : 82, reasons: ["mesmo programa, organização e ciclo"], cycle, tokens };
  }
  ...
  return { key: digest(`role:${company || "unknown"}:${title}:${location}:${cycle || "unknown"}`), confidence: company ? 88 : 60, reasons: ["mesma empresa, cargo, local e ciclo"], cycle, tokens: identityTokens(`${company} ${title}`) };
}
...
export function likelySameOpportunity(a: IdentityInput, b: IdentityInput) {
  const left = buildDedupIdentity(a); const right = buildDedupIdentity(b);
  ...
  const companyA = usefulCompany(a.company || ""); const companyB = usefulCompany(b.company || "");
  if (companyA && companyB && companyA !== companyB) return { same: false, confidence: 0, reason: "empresas diferentes" };
  const shared = left.tokens.filter((token) => right.tokens.includes(token));
  const score = overlap(left.tokens, right.tokens);
  const programs = isProgram(a.title) && isProgram(b.title);
  const distinctiveShared = shared.filter((token) => token.length >= 3);
  const locationA = identityPlain(a.location || ""); const locationB = identityPlain(b.location || "");
  const compatibleLocation = !locationA || !locationB || locationA === locationB || locationA.includes(locationB) || locationB.includes(locationA);
  const sameProgram = programs && (score >= 0.6 || distinctiveShared.length >= 2);
  const sameRole = !programs && Boolean(companyA && companyB) && compatibleLocation && score >= 0.85;
